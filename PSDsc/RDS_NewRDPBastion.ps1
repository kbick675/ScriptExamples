...
{ 
    ...
    Node $NodeName
    {
        ...
        Registry ServerList
        {
            ...
            ValueData = "vm-ist-rdgw1.domain.com"
            ValueType = "String"
        ...
        Registry ntpservers
        {
            ...
            ValueName = "NtpServer"
            ValueData = "ntp3.domain.com,0x01 ntp.domain.com,0x01"
            ValueType = "String"
            Force = $true
        }
...
$scriptblock = {
    if (!((Get-ChildItem Cert:\LocalMachine\My\certthumbprint -ErrorAction SilentlyContinue) -and (Get-ChildItem Cert:\LocalMachine\TrustedPublisher\certthumbprint -ErrorAction SilentlyContinue)))
    {
        Write-Output "Installing DSC Cert"
        $Key = Get-Content C:\pki\key.txt
        $Password = Invoke-RestMethod -Uri "https://passwordstate
        /api/passwords/8764?format=xml" -Method Get -ContentType "application/xml" -Headers @{"apikey"="$Key"}
    ...
    elseif ((Get-ChildItem Cert:\LocalMachine\My\certthumbprint -ErrorAction SilentlyContinue) -and (Get-ChildItem Cert:\LocalMachine\TrustedPublisher\certthumbprint -ErrorAction SilentlyContinue))
    {
...
config_LCM -NodeName $NodeName -OutputPath .\config_LCM -ConfigurationData $config

Set-DscLocalConfigurationManager -ComputerName $NodeName -Path .\config_LCM -Verbose -Force
