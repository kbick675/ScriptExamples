...
url = 'https://bamboo.domain.corp/rest/agents/latest/?uuid={0}'.format(args.ApiKey)
...
for agent in response:
    ...
    if busy == False and online == True:
        ### Disable agent
        disableurl = 'https://bamboo.domain.corp/rest/agents/latest/{0}/state/disable?uuid={1}'.format(idval, args.ApiKey)
        requests.post(disableurl, verify=False)
        ### Verify agent is disabled
        agentcheckurl ='https://bamboo.domain.corp/rest/agents/latest/{0}/state/?uuid={1}'.format(idval, args.ApiKey)
        ...
        agenturl = 'https://bamboo.domain.corp/rest/agents/latest/{0}/capabilities?uuid={1}'.format(idval, args.ApiKey)
        ...
        if ItIsLinux == True and ReadyToClean == True:
            ...
            enableurl = 'https://bamboo.domain.corp/rest/agents/latest/{0}/state/enable?uuid={1}'.format(idval, args.ApiKey)
