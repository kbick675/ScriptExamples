...
if __name__ == '__main__':
  ...
  for agent in agents:
    if agent['online'] and not agent['busy']:
      ...
      if is_enabled(agent):
        print("Error disabling %s. Skipping agent" % agent['name'])
        next
      ...
      if 'clean-me' in caps.keys():
        ...
        if caps['system.distribution.id'] == 'Ubuntu':
          try:
            ...
            print("Error cleaning %s: %s" % (agent['name'], str(err)))
            print("... continuing")
 
      enable_agent(agent)
      if not is_enabled(agent):
        print("Error re-enabling %s after cleaning. Please manually restart." % agent['name'])