...
.PARAMETER AddServiceAccountPasswordToEnigma
Creates a managed service account.
...
.EXAMPLE 
.\New-ServiceAccount.ps1 -name SP2016-SVC -ServiceAccount -AddServiceAccountPasswordToEnigma -EnigmaListID XXXX -EnigmaListApiKey apikeyhere
.EXAMPLE 
.\New-ServiceAccount.ps1 -name SP2016-SVC -GroupManagedServiceAccount -GroupManagedServiceAccountGroup "SP2016-SVCGroup" -DNSName "ht1-wsfc45.default.corp"
.NOTES
...
param (
    ...
    [string]$DNSName,
    [switch]$AddServiceAccountPasswordToEnigma,
    [string]$EnigmaListID,
    [string]$EnigmaListApiKey,
    [switch]$UnixID
)
...
function Add-EnigmaPassword
{
    ...
    $URI = "https://enigma.$($Domain.DNSRoot)/api/passwords/`?format=xml"            
        $Body = @{
        ...
        $Output
}#End function Add-EnigmaPassword
...
if ($ServiceAccount)
{
    ...
    [string]$EnigmaPassword = $UnsecurePassword
    ...
    if ($AddServiceAccountPasswordToEnigma)
    {
        try 
        {
            Add-EnigmaPassword -APIKey $EnigmaListApiKey -PasswordListID $EnigmaListID -Title $name -Password $EnigmaPassword -Username $name
        }
        catch 
        {
            Write-Output "Couldn't add account and password to Enigma. Try again."
            Write-Output "Password is: $($UnsecurePassword)"
            Write-Output "Add account and password to Enigma manually."
            Write-Output "$($Error[0])"
            break
        }
    }
