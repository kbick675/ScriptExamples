...
function Connect-ToExchangeAsSvc
{
    ...
    if ($OpenExchangeSessions -eq $null)
    {
        Write-Output "Establishing new Exchange Session to $($ServerName)."

        $Password = Invoke-RestMethod -Uri "https://enigma/api/passwords/19921?format=xml" -Method Get -ContentType "application/xml" -Headers @{"apikey"="$ApiKey"}
